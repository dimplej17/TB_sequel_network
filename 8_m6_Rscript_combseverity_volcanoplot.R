#########################################
# Volcano plot - combined severity - m6 #
########################################
...
#########################################
m6_MWtest_results <- olink_wilcox(df = merged_OD_IF_data,
                                  variable = 'comb_sev_m6')  

olinkvolcano_plot <- olink_volcano_plot(m6_MWtest_results,
                                        x_lab = "log2FC")
olinkvolcano_plot +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "blue") +
  geom_vline(xintercept = c(-0.5, 0.5), linetype = "dashed", color = "red") +
  #geom_text_repel(data = m6_MWtest_results, aes(label = Assay), size = 3, box.padding = 0.3  , point.padding = 0.3) +
  labs(title = "Differential Expression Analysis at m6 by Lung Function Severity")



olinkvolcano_plot <- ggplot(m6_MWtest_results, aes(x = estimate, y = -log10(Adjusted_pval))) +
  geom_point(data = subset(m6_MWtest_results, abs(estimate) <= 0.5), color = "grey", size = 2, show.legend = FALSE) +
  # Blue points for significant points and large estimates
  geom_point(data = subset(m6_MWtest_results, abs(estimate) > 0.5), color = "blue", size = 2, show.legend = FALSE) +    
  #geom_text_repel(data = m6_MWtest_results, aes(label = Assay), size = 3, box.padding = 0.3, point.padding = 0.3) +  
  geom_text_repel(data = subset(m6_MWtest_results, abs(estimate) > 0.5), aes(label = Assay), size = 3, box.padding = 0.3, point.padding = 0.3) +  # Add labels
  ...
  labs(x = "log2FC", y = "-log10(p-value)", title = "Differential Expression Analysis at m6 by Lung Function Severity") +
  theme_minimal()
...
library(writexl)
write_xlsx(m6_MWtest_results, "/Users/dimplejanardhan/Downloads/LMU Clinic_Student Assistant/R Studio_LMU_Student Assistant_TB/m6_MWtest_results.xlsx")
