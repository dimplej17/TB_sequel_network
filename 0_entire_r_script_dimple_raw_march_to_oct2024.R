...
OD_data <- read_NPX("/Users/dimplejanardhan/Downloads/LMU_Clinic_Student Assistant/IMPORTANT_files_for_project/TB_Sequel_OD_NPX.xlsx")
IF_data <- read_NPX("/Users/dimplejanardhan/Downloads/LMU_Clinic_Student Assistant/IMPORTANT_files_for_project/TB_Sequel_Inflammation_NPX.xlsx")
...
######################################
######################################
# netowrk with actual protein names #
######################################
######################################
...
library(dplyr)
library(tidyr)
library(reshape2)  # Assuming you are using melt from this package

# Threshold for filtering correlations
thr = 0.65
...
# Load the network_with_symbols.csv file
symbol_data <- read.csv("network_with_symbols.csv", stringsAsFactors = TRUE)

# Ensure the columns are correctly named
# The file should have Protein1, Protein2, Correlation, Symbol1, Symbol2
# You can check the column names using colnames(symbol_data)

# Merge the net data with symbol_data to get protein names
#net <- net %>%
  #left_join(symbol_data, by = "Protein1", relationship = "many-to-many") %>%
  #left_join(symbol_data, by = "Protein2", relationship = "many-to-many") %>%
  #rename(Protein1_Name = Symbol1, Protein2_Name = Symbol2)


# Now we have Protein1_Name and Protein2_Name instead of UniProt IDs
adjlist2 <- data.frame(Protein1 = symbol_data$Symbol1, Protein2 = symbol_data$Symbol2, Correlation = symbol_data$Correlation, row.names = NULL)
View(adjlist2)
View(symbol_data)

# Prepare LFC table
lfcTab = data.frame(node = MWtest_results$Assay, lfc = MWtest_results$estimate)
View(MWtest_results)

moduleTab = data.frame(node = geneInfo$Proteins, mod = geneInfo$moduleColor) %>%
  column_to_rownames('node')

# Create graph from data frame
g <- graph_from_data_frame(adjlist, directed = TRUE)

# Set colors based on LFC values
moduleTab = moduleTab[V(g)$name, "mod", drop = FALSE]
moduleTab$mod <- as.factor(moduleTab$mod)

# Get marking groups
markgroups = moduleTab %>%
  rownames_to_column("node") %>%
  group_by(mod) %>%
  group_map(function(x, i) { return(x$node) })
names(markgroups) = moduleTab %>%
  rownames_to_column("node") %>%
  group_by(mod) %>%
  group_map(function(x, i) { return(as.character(i %>% pull)) })

# Prepare LFC table for colors
sublfcTab = lfcTab %>%
  left_join(symbol_data[, c("Protein1", "Symbol1")], by = c("node" = "Protein1")) %>%
  column_to_rownames("node")

V(g)$color <- getColor(sublfcTab[V(g)$name, "lfc"])

# Initialize coordinates for layout
initcoords = moduleTab[V(g)$name, "mod"] %>%
  sapply(function(x) {
    switch(as.character(x), 
           red = c(1., 0.), 
...
coords = qgraph.layout.fruchtermanreingold(e, vcount = vcount(g), groups = markgroups, init = t(initcoords), repulse.rad = 10000)

# Plot the graph with actual protein names as labels
plot(g,
     mark.groups = markgroups,
     mark.col = c(),
     mark.border = names(markgroups),
     ...
     edge.width = 1,
     edge.arrow.size = 0.2,
     vertex.label = V(g)$name)  # Use the actual protein names here
...
# Reading the network_correlations_with_symbols_only.csv file
network_correlations_with_symbols_only <- read.csv("/Users/dimplejanardhan/Downloads/LMU_Clinic_Student Assistant/R_Studio_LMU_Student Assistant_TB/network_correlations_with_symbols_only.csv")

# stacking the Protein.1 & Protein.2 columns into a new coloumn in a new df

df1 <- data.frame(proteins1and2 = c(network_correlations_with_symbols_only$Protein.1, network_correlations_with_symbols_only$Protein.2))
